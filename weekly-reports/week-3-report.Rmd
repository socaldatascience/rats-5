...
neuron_36_spikes_o1 <-
  em_trials %>% 
  filter(trial %in% odor_1_trials) %>% 
  select(time_bin, trial, odor_1, neuron_7) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_7 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
...
neuron_36_spikes_o2 <-
  em_trials %>% 
  filter(trial %in% odor_2_trials) %>% 
  select(time_bin, trial, odor_2, neuron_7) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_7 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
...
neuron_36_spikes_o3 <-
  em_trials %>% 
  filter(trial %in% odor_3_trials) %>% 
  select(time_bin, trial, odor_3, neuron_7) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_7 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
...
neuron_36_spikes_o4 <-
  em_trials %>% 
  filter(trial %in% odor_4_trials) %>% 
  select(time_bin, trial, odor_4, neuron_7) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_7 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
...
neuron_36_spikes_o5 <-
  em_trials %>% 
  filter(trial %in% odor_5_trials) %>% 
  select(time_bin, trial, odor_5, neuron_7) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_7 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
...
# Neuron 3 odors 1 and 3
# Neuron 5 **
# Neuron 6 **
# Neuron 7 **
# Neuron 8 fires at end of trials for odors 1 and 3
# Neuron 9 fires at beginning for odor 1

# Neurons 6 and 7 are the most interesting
...
```{r}
# summary
# Neuron 13 spikes a significant amount during odor 2 towards the middle end of the trials. It doesn't spike much during the other odors
# Neuron 14 fires a plethora of times for all odors. Generally it peaks in the beginning of the trials for all odors.
# Neuron 15 has firing counts all over the place. for odor 2 and 3 it has a significant peak in the beginning towards the middle of the trials. odor 1 peaks middle-end of trial
# Neuron 16 fires a very small amount for all odors. Odor 5 has the most firing counts that are in the beginning and middle of the trial. odor 1 has a high count in middle-end
# Neuron 17 fires a significant amount in odor 1 in the beginning and middle of the trial. The other odors don't have as much firing counts, but still have some
# Neuron 18 has a huge amount of firing counts. Odors 1-5 seem to be firing a great amount in the begging of the trials and it (firing count) gradually decreases as time increases
# Neuron 19 has a surplus of firings for all odors. odor 1 fires all over the place while the rest follow a trend which starts with high firing counts in the beginning of the trial and less in the end.
# Neuron 20 has a very little amount of firing counts. 
# Neuron 21 has very few firing counts for all odors. Insignificant
# Neuron 22 has 0 firing counts for all odors except odor 3. Odor 3 has exactly a firing count of 1 throughout all times in the trials.
# Neuron 23 has zero firing counts for all odors
# Neuron 24 has zero firing counts for all odors
```
...
neuron_25_spikes_o1 <-
  em_trials %>% 
  filter(trial %in% odor_1_trials) %>% 
  select(time_bin, trial, odor_1, neuron_25) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_25 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
  ...
  labs(x = "Time (MS)",
       y = "Firing Count", 
       title = "Firing Counts of Neuron 25 Throughout Trial Run for Odor 1") +
...
neuron_25_spikes_o2 <-
  em_trials %>% 
  filter(trial %in% odor_2_trials) %>% 
  select(time_bin, trial, odor_2, neuron_25) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_25 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
  ...
  labs(x = "Time (MS)",
       y = "Firing Count", 
       title = "Firing Counts of Neuron 25 Throughout Trial Run for Odor 2") +
       scale_y_continuous(limits = c(0,80)) +
       scale_x_continuous(limits = c(0,1.7))
...
neuron_25_spikes_o3 <-
  em_trials %>% 
  filter(trial %in% odor_3_trials) %>% 
  select(time_bin, trial, odor_3, neuron_25) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_25 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
  ...
  labs(x = "Time (MS)",
       y = "Firing Count", 
       title = "Firing Counts of Neuron 25 Throughout Trial Run for Odor 3") +
       scale_y_continuous(limits = c(0,80)) +
       scale_x_continuous(limits = c(0,1.7))
...
neuron_25_spikes_o4 <-
  em_trials %>% 
  filter(trial %in% odor_4_trials) %>% 
  select(time_bin, trial, odor_4, neuron_25) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_25 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
  ...
  labs(x = "Time (MS)",
       y = "Firing Count", 
       title = "Firing Counts of Neuron 25 Throughout Trial Run for Odor 4") +
       scale_y_continuous(limits = c(0,80)) +
       scale_x_continuous(limits = c(0,1.7))
...
neuron_25_spikes_o5 <-
  em_trials %>% 
  filter(trial %in% odor_5_trials) %>% 
  select(time_bin, trial, odor_5, neuron_25) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_25 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
  ...
  labs(x = "Time (MS)",
       y = "Firing Count", 
       title = "Firing Counts of Neuron 25 Throughout Trial Run for Odor 5") +
       scale_y_continuous(limits = c(0,80)) +
       scale_x_continuous(limits = c(0,1.7))


grid.arrange(neuron_25_spikes_o1, 
             neuron_25_spikes_o2, 
             neuron_25_spikes_o3,
             neuron_25_spikes_o4,
             neuron_25_spikes_o5,
...
# For Neuron 28, it's better to use 17 or 18 for y scale.
# For Neuron 34, there's a pattern, but in Odor 1, there are times when it's not fired. Use 50 as y scale.
# For Neuron 35, use 30 as y scale.
# Neuron 29 has a pattern too, use 40 for y scale.
# Neurons 30 - 32 are weird since it is not fired/no activity at all during the time of trial.
```
...
neuron_46_spikes_o1 <-
  em_trials %>% 
  filter(trial %in% odor_1_trials) %>% 
  select(time_bin, trial, odor_1, neuron_46) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_46 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
  ...
  labs(x = "Time (MS)",
       y = "Firing Count", 
       title = "Firing Counts of Neuron 46 Throughout Trial Run for Odor 1") 

neuron_46_spikes_o2 <-
  em_trials %>% 
  filter(trial %in% odor_2_trials) %>% 
  select(time_bin, trial, odor_2, neuron_46) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_46 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
  ...
  labs(x = "Time (MS)",
       y = "Firing Count", 
       title = "Firing Counts of Neuron 46 Throughout Trial Run for Odor 2") 

neuron_46_spikes_o3 <-
  em_trials %>% 
  filter(trial %in% odor_3_trials) %>% 
  select(time_bin, trial, odor_3, neuron_46) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_46 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
  ...
  labs(x = "Time (MS)",
       y = "Firing Count", 
       title = "Firing Counts of Neuron 46 Throughout Trial Run for Odor 3") 

neuron_46_spikes_o4 <-
  em_trials %>% 
  filter(trial %in% odor_4_trials) %>% 
  select(time_bin, trial, odor_4, neuron_46) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_46 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
  ...
  labs(x = "Time (MS)",
       y = "Firing Count", 
       title = "Firing Counts of Neuron 46 Throughout Trial Run for Odor 4") 

neuron_46_spikes_o5 <-
  em_trials %>% 
  filter(trial %in% odor_5_trials) %>% 
  select(time_bin, trial, odor_5, neuron_46) %>% 
  group_by(trial) %>% 
  mutate(time_bin = time_bin - min(time_bin)) %>% 
  filter(neuron_46 == 1) %>% 
  ggplot(aes(x = time_bin)) + 
  ...
  labs(x = "Time (MS)",
       y = "Firing Count", 
       title = "Firing Counts of Neuron 46 Throughout Trial Run for Odor 5") 

# combine plots of the neuron and each odor into one plot
grid.arrange(neuron_46_spikes_o1, 
             neuron_46_spikes_o2, 
             neuron_46_spikes_o3,
             neuron_46_spikes_o4,
             neuron_46_spikes_o5,
...
#neuron 37, 39, 40, 44: no specific pattern. The neuron only fired 1-2 times in react to the odor. 
#neuron 38: Peak at left during odor 4. Relatively inactive in other odors.
#neuron 42: neuron firing during odor 1 (an uniform distribution). Almost none react to the rest of the odor.
#neuron 41: fires up to 122(estimated) during odor 1, has higher peak on the left.
#neuron 43, 45, 46: the neurons have higher peak on the left.
```
