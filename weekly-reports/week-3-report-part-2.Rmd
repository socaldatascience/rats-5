...
train_rows <- sample(all_rows, floor(.9*nrow(odor_e)), replace = F)

train_e <- odor_e[train_rows,]
test_e <- odor_e[-train_rows,]

m5 <- glm(odor ~ neuron_12 + neuron_13 + neuron_14 + neuron_16 + 
            neuron_17 + neuron_18 + neuron_4 + neuron_5 + neuron_6 + 
            neuron_27 + neuron_34 + neuron_35 + neuron_40 + neuron_42 + neuron_44 + neuron_45,
          family = binomial(link = "logit"), 
          data = train_e)
...
predict.fit = m3 %>%  predict(test_e,type="response") # pipe operator run model on train and then run on test data
head(predict.fit)
length(predict.fit)
predicted.classes <- ifelse(predict.fit > 0.5, 1,0) # if in the predict fit data set if the number is greater than 0.5 then it is a 1 otherwise it is a 0 

counter=0
for(i in 1:nrow(test_e))
{
  if(predicted.classes[i] != test_e$odor[i])
  {counter=counter+1}
}
# prediction not equal to actual gender
# if I predicted a 1 as 0 then I misclassified
counter
misclassification.rate=counter/nrow(test_e)
misclassification.rate
