...
library(neuralnet)
>>>>>>> f0bdcaaf848abd56fee64b247a01ed55bc67cf48
```
...
pog_model <- multinom(as.factor(odor) ~ .,
                      data = train)

summary(pog_model)

info <- broom::tidy(pog_model, conf.int = TRUE)
...
test <- 
  test %>% 
  mutate(pred_odor = predict(pog_model, newdata = test, "class"))
...
```{r}
train_pca <- prcomp(train[,c(1:46)], center = TRUE, scale. = FALSE)

summary(train_pca)

# first 10 principal components explain about 80% of the variance
```
